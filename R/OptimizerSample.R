#' @title Sample-Based Optimizer
#'
#' @name mlr_optimizers_sample
#'
#' @description
#' Optimizer that generates a sample of candidates and evaluates them all.
#' This is an extension of [bbotk::OptimizerBatchRandomSearch] that allows
#' for more flexible candidate generation.
#'
#' Related to [bbotk::OptimizerBatchDesignPoints], which uses a fixed design of
#' points to evaluate.
#'
#' @section Parameters:
...
#'
#' @export
OptimizerSample <- R6Class("OptimizerSample",
  ...
  public = list(
    ...
    initialize = function() {
      ...
      param_set <- ps(
        ...
        sample_size = p_int(lower = 1L)  # explicitly not 'required'
      )
      ...
      super$initialize(
        ...
        label = "Sample-Based Optimizer",
        man = "celecx::mlr_optimizers_sample"
      )
    }
  ),
  ...
  private = list(
    ...
    .optimize = function(inst) {
      ...
      sample_size <- pv$sample_size

      if (is.null(sample_size)) {
        # Use remaining evaluations from terminator
        if (!inherits(inst$terminator, "TerminatorEvals")) {
          stop("Terminator must be a TerminatorEvals when no sample_size is set")
        }
        status <- inst$terminator$status(inst$archive)
        sample_size <- status["max_steps"] - status["current_steps"]
      }

      search_space <- inst$search_space
      candidates <- generator(search_space, sample_size)

      # Evaluate all candidates; the instance will handle storing results in the archive
...
#' @include aaa.R
optimizers[["sample"]] = OptimizerSample

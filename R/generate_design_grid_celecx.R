...
celecx_grid_traverse_dependency_aware <- function(param_set,
  ...
  for (param_id in topo_ids) {
    ...
    if (is.finite(upper_limit)) {
      ...
      next_size <- sum(!is_active) +
        sum(is_active) * length(param_grid_values[[param_id]])
...
celecx_grid_expand_active_rows <- function(data, id, values) {
  # Active branches materialize one row per grid value.
...
celecx_grid_mark_inactive_rows <- function(data, id, storage_type) {
  # Inactive branches contribute one typed-NA row.

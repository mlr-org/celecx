...
We propose `batch_size = 10` point in each iteration, which are the top 10 from `acq_evals = 100` candidate points.
We can modify the batch selection further by influencing the `acq_optimizer`: It is an `opt("sample")` optimizer, with its own hyperparameters; here: the sampling method.

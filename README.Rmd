...
We propose `batch_size = 20` point in each iteration, which are the top 20 from `aqf_evals = 200` candidate points.
...
result_data <- result$instance$archive$data[, .(x1, x2, y)]
result_task <- as_task_regr(result_data, target = "y")
kknn_model <- lrn("regr.kknn", k = 3L)$train(result_task)

grid_knn <- data.table::copy(grid)
grid_knn[, y := kknn_model$predict_newdata(grid)$response]

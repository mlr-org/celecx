...
```{r, include = FALSE}
set.seed(1L)
has_kknn <- requireNamespace("kknn", quietly = TRUE)
```
...
```{r example_knn, results = FALSE, message = FALSE, eval = has_kknn}
objective <- ObjectiveRFun$new(
  fun = function(xs) {
...
We propose `batch_size = 10` points in each iteration, which are the top 10 from `acq_evals = 100` candidate points.
We can modify the candidate set further by influencing the `acq_optimizer`.
In the current interface, this is translated to a candidate sampler for the acquisition step; here we use Latin hypercube sampling.

```{r example_knn_opt, results = FALSE, message = FALSE, eval = has_kknn}
